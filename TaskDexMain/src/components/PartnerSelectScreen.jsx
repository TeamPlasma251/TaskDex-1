...
export default function PartnerSelectScreen({ 
  ...
  return (
    <div className="flex flex-col items-center min-h-screen p-6 bg-[#f5f5dc] text-black">
      <div className={style.card + " max-w-7xl w-full mt-6"}>
        <h2 className="main-tab-title mb-8 text-center text-black">Trainer Profile</h2>

        {/* Trainer and Partner Display */}
        <div className="flex justify-center items-center space-x-16 bg-gray-100 p-8 rounded-xl mb-10 border-2 border-gray-300">
          <div className="text-center">
            ...
            />
            <p className="mt-3 text-lg font-semibold">{userData.trainerName}</p>
          </div>
          {currentPartner && (
            <div className="text-center">
              <img 
                ...
                onError={(e) => { e.target.onerror = null; e.target.src = getGifUrl("Placeholder"); }}
              />
              <p className="mt-3 text-lg font-semibold">{currentPartner.currentName}</p>
              {/* EXP Display */}
              {partnerData && partnerData.evoExp !== -1 ? (
                <div className="mt-3">
                  ...
                  <div className="w-full bg-gray-300 rounded-full h-3 mt-2 max-w-[140px] mx-auto">
                    <div 
              ...
              {isReadyToEvolve ? (
                <button 
                  className={style.button + " bg-yellow-500 text-black text-sm py-1 px-3 mt-3 animate-pulse"}
        ...
        <h3 className="text-2xl font-semibold text-black mb-6">My Pokémon ({availablePokemon.length})</h3>
            <div
              className="grid grid-cols-9 md:grid-cols-9 lg:grid-cols-9 gap-6 overflow-y-auto p-6 bg-gray-100 rounded-lg mb-8 border-2 border-gray-300"
          ...
          {availablePokemon.map(mon => {
            ...
            return (
              <div 
                  key={mon.id} 
                  className={`pokemon-card dex-card text-center bg-white rounded-lg cursor-pointer ${typeHoverClass} hover:ring-2 transition-all relative border-2 border-gray-300 hover:scale-105`}
                ...
                <div className="w-full px-2 pb-2">
                  <p className="text-xs font-semibold mt-1 text-center">{mon.currentName}</p>
                  {/* EXP Display */}
                  {evoData && evoData.evoExp !== -1 ? (
                    <div className="mt-2">
                      <p className="exp-text text-center">{Math.floor(mon.exp || 0)} / {evoData.evoExp}</p>
                      <div className="exp-bar-bg mx-auto mt-1">
                        <div className="exp-bar-fill" style={{ width: `${Math.min(expPercent, 100)}%` }}></div>
                      </div>
                    </div>
                  ) : (
                    <p className="text-xs text-gray-500 mt-1 text-center">Max Level</p>
                  )}
                </div>
              </div>
            );
